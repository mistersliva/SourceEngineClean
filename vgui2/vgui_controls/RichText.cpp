...
int RichText::ParseTextStringForUrls( const char *text, int startPos, char *pchURLText, int cchURLText, char *pchURL, int cchURL, bool &clickable )
{
	...
	while (text[i] != 0)
	{
		...
		if ( !Q_strnicmp(text + i, "<a href=", 8) )
		{
			...
			Q_strncpy( pchURL, text + i, ((((int)(pchURLEnd - text) - i + 1) < (cchURL)) ? ((int)(pchURLEnd - text) - i + 1) : (cchURL)) );
			i += ( pchURLEnd - text - i + 1 );
            
			// get the url text
			pchURLEnd = Q_strstr( text, "</a>" );
			Q_strncpy( pchURLText, text + i, ((((int)(pchURLEnd - text) - i + 1) < (cchURLText)) ? ((int)(pchURLEnd - text) - i + 1) : (cchURLText)) );
			i += ( pchURLEnd - text - i );
			i += Q_strlen( "</a>" );
